...
long long calculateAddresszc(long long offset) {
    ...
    if (hasASLRzc()) {
        ...
        long long slide = get_image_vmaddr_slidezc("/angos");
...
bool nssb(long long offset,  unsigned int data) {
    ...
    if(getTypezc(data))
    {
       // data = CFSwapInt32(data);//不改防注释
        err = vm_write(port, address, (long long)&data, sizeof(data));
    }
    else
    {
       // data = (unsigned short)data;//不改防注释
       // data = CFSwapInt16(data);//不改防注释
        err = vm_write(port, address, (long long)&data, sizeof(data));
    }
    if (err != KERN_SUCCESS) {
    return FALSE;
